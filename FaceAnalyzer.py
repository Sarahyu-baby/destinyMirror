...
import numpy as np
from EyeFeatureExtractor import EyeFeatureExtractor
...
class FaceAnalyzer:
    ...
    def _measure_face(self, lms):
        ...
        pts = {
            ...
            'brow_mid': (lms[70] + lms[300]) / 2, # Captures the inner ends of the eyebrows, close to the glabella (between the eyes)
        ...
        measurements = {
            ...
            'upper_lip_h': (dist(pts['mouth_l_top'], pts['mouth_l_mid']) + dist(pts['mouth_r_top'], pts['mouth_r_mid']))/2,
            'lower_lip_h': (dist(pts['mouth_l_bottom'], pts['mouth_l_mid']) + dist(pts['mouth_r_bottom'], pts['mouth_r_mid']))/2,
